...
function Orders() {
  ...
  const fetchAllOrders = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${backendUrl}/api/v1/order/list`, {}, { withCredentials: true })
...
const statusHandler=async(event,orderId)=>{
      try {
        const response=await axios.post(`${backendUrl}/api/v1/order/status`,{orderId,status:event.target.value},{withCredentials:true})
        if (response.data.success){
